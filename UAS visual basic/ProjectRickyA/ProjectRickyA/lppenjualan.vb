...
Public Class lppenjualan
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtcari.Click
        perintah.CommandType = CommandType.Text
        perintah.CommandText = "select * from pemasukan where tanggal between'" & Format(dtp.Value, "yyyy-MM-dd") & "' and '" & Format(DateTimePicker2.Value, "yyyy-MM-dd") & "'"
        mda.SelectCommand = perintah
        ds.Tables.Clear()
        mda.Fill(ds, "pemasukan")
        dgv.DataSource = ds.Tables("pemasukan")
    ...
    Private Sub PD_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PD.BeginPrint
        ...
        pagesetup.PaperSize = New PaperSize("Custom", 740, 600)
    ...
    Private Sub PD_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PD.PrintPage
        ...
        e.Graphics.DrawString("Tanggal", f10b, Brushes.Black, 110, 90)
        e.Graphics.DrawString("ID Item", f10b, Brushes.Black, 250, 90)
        e.Graphics.DrawString("Nama Item", f10b, Brushes.Black, 370, 90)
        e.Graphics.DrawString("Harga", f10b, Brushes.Black, 500, 90)
        e.Graphics.DrawString("Jumlah", f10b, Brushes.Black, 590, 90)
        e.Graphics.DrawString("Total", f10b, Brushes.Black, 670, 90)
        ...
        e.Graphics.DrawString("Total Penjualan : ", f10, Brushes.Black, 530, 570)
        e.Graphics.DrawString(Format(totalpenjualan, "Rp##,##0"), f10, Brushes.Black, 650, 570)
        ...
        For baris As Integer = 0 To dgv.RowCount - 1
            ...
            e.Graphics.DrawString(dgv.Rows(baris).Cells(2).Value.ToString, f10, Brushes.Black, 110, 105 + tinggi)
            e.Graphics.DrawString(dgv.Rows(baris).Cells(1).Value.ToString, f10, Brushes.Black, 250, 105 + tinggi)
            e.Graphics.DrawString(dgv.Rows(baris).Cells(3).Value.ToString, f10, Brushes.Black, 370, 105 + tinggi)
            e.Graphics.DrawString(dgv.Rows(baris).Cells(4).Value.ToString, f10, Brushes.Black, 500, 105 + tinggi)
            e.Graphics.DrawString(dgv.Rows(baris).Cells(5).Value.ToString, f10, Brushes.Black, 600, 105 + tinggi)
            e.Graphics.DrawString(dgv.Rows(baris).Cells(6).Value.ToString, f10, Brushes.Black, 670, 105 + tinggi)
        Next
    End Sub
